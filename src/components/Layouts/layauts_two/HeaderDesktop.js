...
const HeaderDesktop = ({LinksNavDesktop, logo}) => {
    ...
    const {openMenuDesktop, setOpenMenuDesktop,openSubsubNav, setOpenSubsubNav}=useContext(PaginaContextValue)
    useEffect(() => {
      const onClickOutsideHandler = event => {
        if (openMenuDesktop && !toggleContainer.current.contains(event.target)) {
          setOpenMenuDesktop(false);
        }
        if (openSubsubNav && !toggleContainer.current.contains(event.target)) {
...
console.log("opensubnavd",openSubsubNav)
    ...
    return (
      <div ref={toggleContainer}>
        <div className={`bg-black z-[40] lg:w-full lg:h-[80px] lg:flex lg:justify-center lg:items-center`}>
